...
(define-public (unwrap-cairo_pdf_version_t n)
  (cond
   ((symbol? n)
    (or (assq-ref cairo_pdf_version_t-enum-nvl n)
        (throw 'ffi-help-error "bad arg: ~A" n)))
   ((integer? n) n)
   (else (error "bad arg"))))
...
(define-public (unwrap-cairo_pdf_outline_flags_t n)
  (cond
   ((symbol? n)
    (or (assq-ref cairo_pdf_outline_flags_t-enum-nvl n)
        (throw 'ffi-help-error "bad arg: ~A" n)))
   ((integer? n) n)
   (else (error "bad arg"))))
...
;; int cairo_pdf_surface_add_outline(cairo_surface_t *surface, int parent_id, 
;;     const char *utf8, const char *link_attribs, cairo_pdf_outline_flags_t 
;;     flags);
...
(define-public (unwrap-cairo_pdf_metadata_t n)
  (cond
   ((symbol? n)
    (or (assq-ref cairo_pdf_metadata_t-enum-nvl n)
        (throw 'ffi-help-error "bad arg: ~A" n)))
   ((integer? n) n)
   (else (error "bad arg"))))
...
;; void cairo_pdf_surface_set_page_label(cairo_surface_t *surface, const char *
;;     utf8);
...
;; void cairo_pdf_surface_set_thumbnail_size(cairo_surface_t *surface, int 
;;     width, int height);
